...
const definition = defineWorkflow<Place, Record<string, unknown>>({
  ...
  transitions: [
    { name: "start", type: "automatic", inputs: ["idle"], outputs: ["processing"] },
    { name: "finish", type: "automatic", inputs: ["processing"], outputs: ["done"] },
  ],
...
describe("analyse", () => {
  ...
  it("analyses deadlock-free net", () => {
    ...
    const cycleDef = defineWorkflow<CyclePlace, Record<string, unknown>>({
      ...
      places: ["a", "b"],
      transitions: [
        { name: "forward", type: "automatic", inputs: ["a"], outputs: ["b"] },
        { name: "back", type: "automatic", inputs: ["b"], outputs: ["a"] },
      ],
